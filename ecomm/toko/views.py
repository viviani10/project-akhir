...
# class ProductKategoriView(View):
#     def get(self, request, kategori):
#         # items = Item.objects.filter(kategori=kategori).values('nama_produk').first()
#         # queryset = ProdukItem.objects.all()
#         items = Item.objects.filter(kategori=kategori).order_by('nama_produk')
#         if items.exists():
#             context = {
#                 'kategori': kategori,
#                 'items' : items,
...
#         template_name = 'tampilan_kategori.html'
#         print(items)
#         # paginate_by = 4
...
# class ProductKategoriView(View):
#     def get(self, request, kategori):
#         items = Item.objects.filter(kategori=kategori)
#         context = {'items': items, 'kategori': kategori}
...
class ProductKategoriView(View):
    @staticmethod
    def get_kategori_display(kategori):
        PILIHAN_KATEGORI = ProdukItem.PILIHAN_KATEGORI
        for choice in PILIHAN_KATEGORI:
            if choice[0] == kategori:
                return choice[1]
        return ''

    def get(self, request, kategori):
        items = ProdukItem.objects.filter(kategori=kategori).order_by('nama_produk')
        if items.exists():
            context = {
                'kategori': kategori,
                'kategori_display': self.get_kategori_display(kategori),
                'items': items,
            }
        else:
            context = {
